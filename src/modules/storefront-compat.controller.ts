...
export class StorefrontCompatController {
  ...
  async getProductBySlug(@Param('slug') slug: string, @Query('country') country?: string, @Query('host') host?: string) {
    ...
    return {
      ...
      currency: product.baseCurrency || 'TRY',
      inStock: product.hasInfiniteStock || product.stockCount > 0,
      imageUrl: this.normalizeImageUrl(product.iconUrl || product.category?.imageUrl || product.merchantImageUrl, product.category?.slug || product.slug),
      merchantImageUrl: this.normalizeImageUrl(product.merchantImageUrl || product.sliderImageUrl || product.category?.imageUrl || product.iconUrl, product.category?.slug || product.slug),
      sliderImageUrl: this.normalizeImageUrl(product.sliderImageUrl || product.merchantImageUrl || product.iconUrl || product.category?.imageUrl, product.category?.slug || product.slug),
      categoryName: product.category?.name || '',
      categoryId: product.categoryId,
      ...
      relatedProducts: visibleRelatedProducts.map((item: any) => {
        ...
        return {
          ...
          imageUrl: this.normalizeImageUrl(item.iconUrl || item.category?.imageUrl || item.merchantImageUrl || product.category?.imageUrl, item.category?.slug || item.slug),
